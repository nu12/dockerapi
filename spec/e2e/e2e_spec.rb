RSpec.describe "End-to-end test", e2e: true do
    let(:image_ref) { "nginx@sha256:94f1c83ea210e0568f87884517b4fe9a39c74b7677e0ad3de72700cfa3da7268" }
    before(:all) do
        
    end
    after(:all) do
        Docker::API::Image.new.remove("localhost/dockerapi", force: true)
        Docker::API::Image.new.remove(image_ref, force: true)
    ...
    describe "Basic workflow" do 
        it "downloads an image" do expect(image.create( fromImage: image_ref ).status).to be(200)  end
        it "inspects the image" do expect(image.details( image_ref ).json).to be_kind_of(Hash) end
        it "tags the image" do  expect(image.tag(image_ref, repo: "localhost/dockerapi").status).to be(201) end
        it "removes an image" do expect(image.remove(image_ref).status).to be(200) end
